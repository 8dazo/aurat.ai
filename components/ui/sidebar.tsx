...
>(
    (
        ...
        // We add a state so that we can use it to style the children components.
...
>(
    (
        ...
        if (isMobile) {
            // Mobile implementation would use a Sheet or similar, keeping it simple for now
            return null;
        }
...
>(({ className, onClick, ...props }, ref) => {
    ...
    return (
        <button
            ...
            className={cn('h-7 w-7 outline-none', className)}
            onClick={(event) => {
                onClick?.(event);
                toggleSidebar();
            }}
